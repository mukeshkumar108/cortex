...
async def _memory_query_legacy_adapter(request: MemoryQueryRequest) -> MemoryQueryResponse:
    ...
    Kept as explicit temporary compatibility boundary while rollout/rollback controls remain active.
    """
...
async def debug_graphiti_query(
    ...
    _require_internal_token(x_internal_token)
    # T15 deprecation: legacy Graphiti-era debug retrieval path is intentionally disabled.
    raise HTTPException(
        status_code=410,
        detail=(
            "Deprecated endpoint: /internal/debug/graphiti/query is disabled. "
            "Use /v2/memory/query for retrieval validation and /internal/v2/retrieval-shadow/audit for diff visibility."
        ),
    )
