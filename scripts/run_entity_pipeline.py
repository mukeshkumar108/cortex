#!/usr/bin/env python3
"""Run Pass 1.5 entity updates using the shared live pass implementation."""
...
from src.derived_pipeline import run_pass1_5_entities
...
async def run_batch(*, user_id: str, tenant_id: str = "default", limit: int = 100) -> Dict[str, int]:
    ...
    try:
        rows = await db.fetch(
            ...
            WHERE sc.user_id=$1 AND COALESCE(st.tenant_id, 'default')=$2 AND sc.run_entity_pass IS TRUE
